        ...
        <div class="col-12 col-lg-4 col-xxl">
            <div class="vironeer-counter-card bg-c-7">
                ...
                <div class="vironeer-counter-card-meta">
                    <p class="vironeer-counter-card-title">{{ __('Waiting review') }}</p>
                    <p class="vironeer-counter-card-number">{{ formatNumber($waitingReview) }}</p>
                </div>
            </div>
        </div>
        <div class="col-12 col-lg-4 col-xxl">
            <div class="vironeer-counter-card bg-c-4">
                ...
                <div class="vironeer-counter-card-meta">
                    <p class="vironeer-counter-card-title">{{ __('Reviewed') }}</p>
                    <p class="vironeer-counter-card-number">{{ formatNumber($reviewed) }}</p>
                </div>
            </div>
        </div>
    </div>
                        ...
                        <td>
                            <div class="vironeer-content-box">
                                <a class="vironeer-content-image text-center"
                                    href="{{ route($fileReport->fileEntry->user_id ? 'superadmin.uploads.users.view' : 'superadmin.uploads.guests.view', $fileReport->fileEntry->shared_id) }}">
                                    {{-- @if ($fileReport->fileEntry->type == 'image')
                                        <img src="{{ route('superadmin.uploads.secure', hashid($fileReport->fileEntry->id)) }}" alt="{{ $fileReport->fileEntry->name }}">
                                    @else
                                ...
                                <ul class="dropdown-menu dropdown-menu-sm-end dropdown-menu-lg"
                                    ...
                                    <li>
                                        <a class="dropdown-item"
                                            href="{{ route('superadmin.reports.view', $fileReport->id) }}"><i
                                    ...
                                    <li>
                                        <form action="{{ route('superadmin.reports.destroy', $fileReport->id) }}" method="POST">
                                            @csrf
                                            @method('DELETE')
