...
@section('back', route('superadmin.reports.index'))
        ...
        <div class="card-body">
            <div class="d-flex align-items-center">
                <div class="flex-shrink-0">
                    <a href="{{ route($fileReport->fileEntry->user_id ? 'superadmin.uploads.users.view' : 'superadmin.uploads.guests.view', $fileReport->fileEntry->shared_id) }}"
                        target="_blank">
                        @if ($fileReport->fileEntry->type == 'image')
                            <img class="rounded-2"
                                src="{{ route('superadmin.uploads.secure', hashid($fileReport->fileEntry->id)) }}"
        ...
        <div class="card-body">
            <div class="row g-3">
                ...
                @if (!$fileReport->admin_has_viewed)
                    <div class="col-lg-6">
                        <form action="{{ route('superadmin.reports.markAsReviewed', $fileReport->id) }}" method="POST">
                            @csrf
