...
@section('link', route('superadmin.navbarMenu.create'))
...
@section('content')
    @if ($navbarMenuLinks->count() > 0)
        <form id="vironeer-submited-form" action="{{ route('superadmin.navbarMenu.sort') }}" method="POST">
            @csrf
                    ...
                    <li data-id="{{ $navbarMenuLink->id }}"
                        ...
                        <div class="buttons">
                            <a class="btn btn-blue btn-sm me-2"
                                href="{{ route('superadmin.navbarMenu.edit', $navbarMenuLink->id) }}"><i
                                    class="fa fa-edit"></i></a>
                            <form class="d-inline"
                                action="{{ route('superadmin.navbarMenu.destroy', $navbarMenuLink->id) }}" method="POST">
                                @method('DELETE')
                                @csrf
