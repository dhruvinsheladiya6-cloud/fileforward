<div class="custom-card card mb-3">
    <div class="card-body text-center">
        ...
        <div class="buttons">
            ...
            <form class="d-inline" action="{{ route('superadmin.users.deleteAvatar', $user->id) }}" method="POST">
                @csrf
                @method('DELETE')
...
<div class="custom-card card mb-3">
    <div class="custom-list-group list-group list-group-flush">
        <a href="{{ route('superadmin.users.edit', $user->id) }}"
            class="list-group-item list-group-item-action d-flex justify-content-between @if (request()->routeIs('superadmin.users.edit')) active @endif">
            <span><i class="fa fa-edit me-2"></i>{{ __('Account details') }}</span>
            <span><i class="fas fa-angle-right"></i></span>
        </a>
        <a href="{{ route('superadmin.users.logs', $user->id) }}"
            class="list-group-item list-group-item-action d-flex justify-content-between @if (request()->routeIs('superadmin.users.logs')) active @endif">
            <span><i class="fas fa-list-ul me-2"></i>{{ __('Login logs') }}</span>
            <span><i class="fas fa-angle-right"></i></span>
        </a>
        @if (licenseType(2) && $subscription)
            <a href="{{ route('superadmin.subscriptions.edit', $subscription->id) }}"
                class="list-group-item list-group-item-action d-flex justify-content-between">
                <span><i class="far fa-gem me-2"></i>{{ __('Subscription') }}</span>
                <span><i class="fas fa-angle-right"></i></span>
            </a>
        @endif
        <a href="{{ route('superadmin.uploads.users.index') . '?user=' . hashid($user->id) }}"
            class="list-group-item list-group-item-action d-flex justify-content-between">
