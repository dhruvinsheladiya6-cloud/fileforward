...
@section('back', route('superadmin.subscriptions.index'))
...
@section('content')
    ...
    <div class="card text-center mb-3 p-4">
        <div class="card-body">
            ...
            <a href="{{ route('superadmin.users.edit', $subscription->user->id) }}"
                class="btn btn-dark">{{ __('View details') }}</a>
        </div>
    </div>
    <form id="vironeer-submited-form" action="{{ route('superadmin.subscriptions.update', $subscription->id) }}" method="POST">
        @csrf
        @method('PUT')
        <div class="card p-2">
            <div class="card-body">
