...
@section('back', route('superadmin.uploads.users.index'))
@section('content')
    <div class="card custom-card mb-2">
        <div class="card-body">
            <div class="d-flex align-items-center">
                <div class="flex-shrink-0">
                    <a href="{{ route('superadmin.users.edit', $fileEntry->user->id) }}">
                        <img class="border rounded-circle border-2" src="{{ asset($fileEntry->user->avatar) }}" width="60"
                            height="60">
                    </a>
                </div>
                <div class="flex-grow-1 ms-3">
                    <a href="{{ route('superadmin.users.edit', $fileEntry->user->id) }}" class="text-dark">
                        <h5 class="mb-1">
                ...
                <div class="flex-grow-3 ms-3">
                    <a href="{{ route('superadmin.users.edit', $fileEntry->user->id) }}" class="btn btn-dark" target="_blank"><i
            ...
            <div class="card">
                <ul class="custom-list-group list-group list-group-flush">
                    <li class="list-group-item">
                        ...
                        <a href="{{ route('superadmin.uploads.users.download', $fileEntry->shared_id) }}"
                            class="btn btn-success btn-lg w-100 mb-3"><i
                                class="fas fa-download me-2"></i>{{ __('Download') }}</a>
                        <form action="{{ route('superadmin.uploads.users.destroy', $fileEntry->shared_id) }}" method="POST">
                            @csrf @method('DELETE')
