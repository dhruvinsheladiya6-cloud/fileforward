...
@section('title', __('Login Logs for ' . request()->ip))
@section('back', route('superadmin.users.index'))
@section('content')
            ...
            @forelse ($logs as $log)
                <div class="logs-box justify-items-center">
                    <div class="content ms-1 d-flex justify-content-between">
                        <span>
                            <h5><a href="{{ route('superadmin.users.logsbyip', $log->ip) }}">{{ $log->ip }}</a></h5>
                            <p class="text-muted capitalize"><i class="fas fa-map-marker-alt me-2"></i>{{ $log->location }}
                                <span class="me-1 ms-1">|</span> <i class="fa fa-user me-1"></i> <a
                                    href="{{ route('superadmin.users.edit', $log->user->id) }}">{{ $log->user->username }}</a>
                            </p>
                        </span>
                        <span>
