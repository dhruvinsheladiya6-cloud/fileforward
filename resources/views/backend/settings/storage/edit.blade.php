...
@section('back', route('superadmin.settings.storage.index'))
@section('container', 'container-max-lg')
@section('content')
    <form id="vironeer-submited-form" action="{{ route('superadmin.settings.storage.update', $storageProvider->id) }}"
        method="POST">
        @csrf
    ...
    @if ($storageProvider->status)
        <div class="card custom-card">
            ...
            <div class="card-body">
                <form action="{{ route('superadmin.settings.storage.test', $storageProvider->id) }}" method="POST">
                    @csrf
