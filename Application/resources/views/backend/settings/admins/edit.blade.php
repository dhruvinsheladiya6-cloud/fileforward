...
@section('content')
    <form id="vironeer-submited-form" action="{{ route('admins.update', $admin->id) }}" method="POST"
        ...
        <div class="card p-2">
            <div class="card-body">
                ...
                @php $isSelf = $admin->id == adminAuthInfo()->id; @endphp
                <div class="mb-3">
                    <label class="form-label">{{ __('Role') }} :</label>
                    <select name="role" class="form-select" {{ $isSelf ? 'disabled' : '' }}>
                        <option value="admin" {{ ($admin->role ?? 'admin') === 'admin' ? 'selected' : '' }}>
                            {{ __('Admin') }}
                        </option>
                        <option value="report_admin" {{ ($admin->role ?? 'admin') === 'report_admin' ? 'selected' : '' }}>
                            {{ __('Report Admin') }}
                        </option>
                    </select>
                    @if($isSelf)
                        {{-- keep existing role when disabled --}}
                        <input type="hidden" name="role" value="{{ $admin->role ?? 'admin' }}">
                        <small class="text-muted">{{ __("You can't change your own role.") }}</small>
                    @endif
                </div>
                <div class="mb-3">
                    <label class="form-label">{{ __('New Password') }} : </label>
                    <input type="text" class="form-control" name="password">
                    <small class="text-muted">{{ __("Leave it empty if you don't want to change password") }}</small>
                </div>
            </div>
        </div>
    </form>
