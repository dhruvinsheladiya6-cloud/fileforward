                ...
                <div class="py-4 g-4">
                    <div class="row align-items-center">
                        ...
                        <div class="col-auto">
                            ...
                            @endif
                            @if (request()->routeIs('superadmin.advertisements.index'))
                                <a href="{{ route('superadmin.advertisements.edit', $headAd->id) }}" class="btn btn-blue"><i
                                        class="fa fa-code me-2"></i>{{ __('Head Code') }}</a>
                            @endif
                            @if (request()->routeIs('superadmin.transactions.edit'))
                                ...
                                @endif
                            @endif
                            @if (request()->routeIs('superadmin.notifications.index'))
                                <a class="vironeer-link-confirm btn btn-outline-success me-2"
                                    href="{{ route('superadmin.notifications.readall') }}">{{ __('Make All as Read') }}</a>
                                <form class="d-inline" action="{{ route('superadmin.notifications.deleteallread') }}"
                                    method="POST">
                                    @csrf
                                    @method('DELETE')
