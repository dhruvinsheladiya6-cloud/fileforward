...
@section('link', route('superadmin.footerMenu.create'))
...
@section('content')
    @if ($footerMenuLinks->count() > 0)
        <form id="vironeer-submited-form" action="{{ route('superadmin.footerMenu.sort') }}" method="POST">
            @csrf
            ...
            <ul class="vironeer-sort-menu custom-list-group list-group list-group-flush">
                @foreach ($footerMenuLinks as $footerMenuLink)
                    <li data-id="{{ $footerMenuLink->id }}"
                        ...
                        <div class="buttons">
                            <a href="{{ route('superadmin.footerMenu.edit', $footerMenuLink->id) }}"
                                class="vironeer-edit-footer-menu btn btn-blue btn-sm me-2"><i class="fa fa-edit"></i></a>
                            <form class="d-inline"
                                action="{{ route('superadmin.footerMenu.destroy', $footerMenuLink->id) }}" method="POST">
                                @method('DELETE')
                                @csrf
