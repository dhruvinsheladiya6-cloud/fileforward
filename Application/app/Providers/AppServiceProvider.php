        ...
        if (config('vironeer.install.complete')) {
            ...
            if (request()->segment(1) != "superadmin") {
                ...
                view()->composer(['frontend.user.layouts.dash'], function ($view) {
                    ...
                    $view->with(['countPlans' => $countPlans]);
                });

                view()->composer(['frontend.user.includes.notification-menu', 'frontend.user.layouts.dash'], function ($view) {
            ...
            if (request()->segment(1) == "superadmin") {
                ...
                view()->composer('backend.includes.sidebar', function ($view) {
                    ...
                    $usersUnviewedUsersCount = FileEntry::where('admin_has_viewed', 0)->userEntry()->notExpired()->count();
                    $guestsUnviewedUsersCount = FileEntry::where('admin_has_viewed', 0)->guestEntry()->notExpired()->count();
