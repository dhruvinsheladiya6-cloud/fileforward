...
class SubscriptionManager
{
    ...
    protected static function filesDuration($filesDuration)
    {
    ...
    {
        if (Auth::user()) {
            $usedSpace = FileEntry::where(function ($query) {
                // Count files uploaded by the user via shared folders
                $query->where('uploaded_by', Auth::user()->id)
                    ->whereNotNull('uploaded_via_share_id')
                    // Count files owned by the user (direct uploads)
                    ->orWhere(function ($subQuery) {
                        $subQuery->where('user_id', Auth::user()->id)
                                ->whereNull('uploaded_via_share_id');
                    });
            })
            ->whereNull('purge_at')
            ->sum('size');
        } else {
            $usedSpace = FileEntry::where([['ip', vIpInfo()->ip], ['user_id', null]])
                                ->whereNull('purge_at')
                                ->sum('size');
        }
    ...
    protected static function storageFullness($plan)
    {
        ...
        return round($fullnessPercentage, 2);
    }
