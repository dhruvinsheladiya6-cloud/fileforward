...
function showPlansByInterval($interval)
{
    $plans = Plan::where('interval', $interval)->get();
    return $plans;
}
...
{
    $subscription = (Auth::user()) ? SubscriptionManager::registredUserSubscriptionDetails(userAuthInfo()) : SubscriptionManager::unregistredUserSubscriptionDetails();
...
function formatInterval($interval)
{
    if ($interval == 1) {
        ...
        return lang('lifetime');
    } else {
        return '';
    }
}

function planIntervalName($interval)
{
    if ($interval == 1) {
        return lang('month', 'plans');
    } elseif ($interval == 2) {
        return lang('year', 'plans');
    } elseif ($interval == 3) {
...
{
    if ($action == 1) {
        return lang('Subscribe', 'transactions');
    } elseif ($action == 2) {
        return lang('Renew', 'transactions');
    } elseif ($action == 3) {
        return lang('Upgrade', 'transactions');
    }
}
...
{
    if ($feature) {
        return '<div class="plan-feature-icon"> <i class="fa fa-check fa-sm"></i> </div>';
    } else {
        return '<div class="plan-feature-icon icon-danger"> <i class="fa fa-times fa-sm"></i> </div>';
    }
}
...
function planButton($plan)
{
    ...
    if (Auth::user() || $plan->require_login) {
        if (Auth::user() && !is_null(userAuthInfo()->subscription) &&
            $plan->id == userAuthInfo()->subscription->plan->id && userAuthInfo()->subscription->plan->free_plan ||
            Auth::user() && !is_null(userAuthInfo()->subscription) &&
            $plan->id == userAuthInfo()->subscription->plan->id && $plan->interval == 3) {
            $button = '<div class="plan-action"></div>';
        } else {
            $buttonText = lang('Choose plan', 'plans');
            $type = "subscribe";
            if (Auth::user()) {
                if (!is_null(userAuthInfo()->subscription)) {
                    if ($plan->id == userAuthInfo()->subscription->plan->id) {
            ...
            $buttonClass = ($plan->free_plan) ? 'btn-outline-primary' : 'btn-primary';
            $confirmClass = (Auth::user()) ? 'confirm-action-form' : '';
            $url = route('subscribe', [hashid($plan->id), $type]);
            $token = csrf_token();
            $button = '<div class="plan-action"><form action="' . $url . '" method="POST"><input type="hidden" name="_token" value="' . $token . '">
            <button class="btn ' . $buttonClass . ' w-100 ' . $confirmClass . ' py-2">' . $buttonText . '</button></form></div>';
        }
    }
    return $button;
}
...
function planBadge($plan)
{
    ...
    if (Auth::user() && !is_null(userAuthInfo()->subscription) && $plan->id == userAuthInfo()->subscription->plan_id) {
        $badge['name'] = '<div class="plan-badge"><span>' . lang('Your plan', 'plans') . '</span></div>';
        $badge['class'] = "current-plan";
    } elseif ($plan->featured_plan) {
        $badge['name'] = (Auth::user() && !is_null(userAuthInfo()->subscription) && $plan->id == userAuthInfo()->subscription->plan->id)
        ? '<div class="plan-badge"><span>' . lang('Your plan', 'plans') . '</span></div>' : '<div class="plan-badge"><span>' . lang('Featured', 'plans') . '</span></div>';
        $badge['class'] = "plan-featured";
    }
    return json_decode(json_encode($badge));
}
...
function autoDeletePeriods()
{
    ...
    if (!is_null(subscription()->plan->files_duration)) {
        $periods = [];
        foreach ($arr as $key => $value) {
            if ($value['days'] > 0 && $value['days'] <= subscription()->plan->files_duration) {
                $periods[] = $value;
            }
        }
        return $periods;
...
function unacceptableFileTypes()
{
    $array = explode(',', settings('unacceptable_file_types'));
...
function getFileType($mimeType)
{
    ...
    if (in_array($mimeType, $imageMimeTypes)) {
        ...
        return "document";
    } elseif (in_array($mimeType, $archiveMimeTypes)) {
        return "archive";
...
function changeChunkFileName($file)
{
    $fileExtension = $file->getClientOriginalExtension();
    $fileName = $file->getClientOriginalName();
    $fileNoExt = str_replace('.' . $fileExtension, '', $fileName);
...
// function isFileSupportPreview($fileType)
// {
...
//     return false;
// }
function isFileSupportPreview($type, $extension = null) {
    $extension = $extension ? strtolower($extension) : strtolower($type);
    ...
    $supportedTypes = [
        ...
        'mp3', 'wav', 'ogg', 'aac', 'm4a',
        // Text
        'txt', 'html', 'css', 'js', 'php', 'py', 'java', 'cpp', 'c', 'json', 'xml', 'md', 'log',
        'zip','rar','7z','tar','gz',
...
{
    return '<a href="https://www.facebook.com/sharer/sharer.php?u=' . $link . '" target="_blank" class="bg-facebook"><i class="fab fa-facebook-f"></i></a>
    <a href="https://twitter.com/intent/tweet?text=' . $link . '" target="_blank" class="bg-twitter"><i class="fab fa-twitter"></i></a>
    <a href="https://wa.me/?text=' . $link . '" target="_blank" class="bg-whatsapp"><i class="fab fa-whatsapp"></i></a>
    <a href="https://www.linkedin.com/shareArticle?mini=true&url=' . $link . '" target="_blank" class="bg-linkedin"><i class="fab fa-linkedin"></i></a>
    <a href="http://pinterest.com/pin/create/button/?url=' . $link . '" target="_blank" class="bg-pinterest"><i class="fab fa-pinterest"></i></a>';
}

function fileDescription($fileEntery)
{
    $replaceFrom = ['{website_name}', '{filename}', '{file_extension}'];
    $replaceTo = [settings('website_name'), $fileEntery->name, $fileEntery->extension];
    $autoDescription = lang('File name {filename}, which is a {file_extension} format, and it its one of the files that can be downloaded and used easily. You can upload similar files without the need for a {website_name} account, or you can create an account by clicking on the sign-up button and you will be able to upload and manage your files. {website_name} supports many file formats and you can upload your files and share them anywhere and download them anytime', 'download page');
    return str_replace($replaceFrom, $replaceTo, $autoDescription);
}
...
{
    $reasons = [
        0 => lang('Privacy, copyright or legal complaints', 'download page'),
        1 => lang('Spam or misleading', 'download page'),
        2 => lang('Malware, virus or malicious content', 'download page'),
        3 => lang('Child abuse', 'download page'),
        4 => lang('Other', 'download page'),
    ];
    return $reasons;
}

function isAddonExists($symbol)
{
    $addon = Addon::where('symbol', $symbol)->active()->first();
    if ($addon) {
        return true;
    }
    return false;
